...
void gui::Render() noexcept
{
	...
	if (globals::glow)
	{
		...
		ImGui::Checkbox("Team Glow", &globals::glowTeam);
	}
	...
	if (globals::chams)
	{
		ImGui::ColorEdit3("Friendly color", globals::chamColorEnemy);
		ImGui::ColorEdit3("Enemy color", globals::chamColorFriendly);
	...
	if (globals::aimbot)
	{
		...
		if (ImGui::BeginCombo("##combo", globals::currentItem)) // The second parameter is the label previewed before opening the combo.
		{
			...
			{
				bool is_selected = (globals::currentItem == items[n]); // You can store your selection however you want, outside or inside your objects
				if (ImGui::Selectable(items[n], is_selected))
					globals::currentItem = items[n];
					auto hitBox = globals::currentItem;
				if (is_selected)
					//hitBox = globals::currentItem;
	...
	ImGui::Checkbox("Recoil", &globals::recoil);
